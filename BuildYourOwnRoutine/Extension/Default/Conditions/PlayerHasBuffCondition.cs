    ...
    internal class PlayerHasBuffCondition : ExtensionCondition
    {
        ...
        public override Func<bool> GetCondition(ExtensionParameter extensionParameter)
        {
            return () =>
            {
                ...
                {
                    var foundBuff = extensionParameter.Plugin.GameController.Game.IngameState.Data.LocalPlayer.GetComponent<ExileCore.PoEMemory.Components.Buffs>().BuffsList.FirstOrDefault(x =>
                        x.Name == HasBuffReady);
                    if (foundBuff != null)
                    {
                ...
                return extensionParameter.Plugin.GameController.Game.IngameState.Data.LocalPlayer.GetComponent<ExileCore.PoEMemory.Components.Buffs>().BuffsList.Any(x => x.Name == HasBuffReady && (x.Timer >= (1.0 * RemainingDuration / 1000)) && x.Charges >= MinimumCharges);
            };
        }
